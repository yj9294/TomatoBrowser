...
//  TomatoBrowser
//
//  Created by yangjian on 2023/10/26.
//
...
extension GADUtil {
    ...
    public func show(_ position: GADPosition, from vc: UIViewController? = nil , preload: (()->Void)? = nil, completion: ((GADBaseModel?)->Void)? = nil) {
        // 超限需要清空广告
        if isGADLimited {
            clean(.native)
            clean(.interstitial)
...
extension GADOpenModel: GADFullScreenContentDelegate {
    override func loadAd(completion: ((_ result: Bool, _ error: String) -> Void)?) {
        ...
        GADAppOpenAd.load(withAdUnitID: model?.theAdID ?? "", request: GADRequest(), orientation: orientation) { [weak self] ad, error in
            guard let self = self else { return }
            if let error = error {
                NSLog("[AD] (\(self.position.rawValue)) load ad FAILED for id \(self.model?.theAdID ?? "invalid id")")
                self.loadedHandler?(false, error.localizedDescription)
...
extension GADUtil {
    ...
    public func show(_ position: GADPosition) async -> GADBaseModel? {
        return await withCheckedContinuation { continuation in
            show(position, completion:  { model in
...
extension GADLoadModel {
    ...
    func prepareLoadAd(_ array: [GADModel], at index: Int = 0)  async throws -> GADBaseModel? {
        ...
        NSLog("[AD] (\(position)) prepareLoaded.")
        if GADUtil.share.isGADLimited {
            NSLog("[AD] (\(position.rawValue)) loade ad limited.")
            throw GADPreloadError.isLimited
        }
        if isPreloadedAD {
            NSLog("[AD] (\(position.rawValue)) load ad completion.")
            return loadedArray.first
        }
        if isPreloadingAD {
            NSLog("[AD] (\(position.rawValue)) loading ad.")
        ...
        guard let ad = ad  else {
            NSLog("[AD] (\(position.rawValue)) ad posion error.")
            throw GADPreloadError.config
        }
        ad.position = position
        loadingArray.append(ad)
